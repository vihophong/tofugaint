...
//  G4cout<<pmtNumber<<G4endl;
  ...
  hit->AddT(aStep->GetPostStepPoint()->GetGlobalTime());
  hit->SetTmin(aStep->GetPostStepPoint()->GetGlobalTime());
  hit->AddPhysPos(aStep->GetPostStepPoint()->GetPosition().getX(),aStep->GetPostStepPoint()->GetPosition().getY(),aStep->GetPostStepPoint()->GetPosition().getZ());
  hit->SetTi(aStep->GetPostStepPoint()->GetGlobalTime());
  hit->IncPhotonCount();  // increment hit for the selected pmt
  G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
  if (pmtNumber<2)
      analysisManager->FillH2(pmtNumber,aStep->GetPostStepPoint()->GetPosition().getY(),aStep->GetPostStepPoint()->GetPosition().getZ());
  else
      analysisManager->FillH2(pmtNumber,aStep->GetPostStepPoint()->GetPosition().getX(),aStep->GetPostStepPoint()->GetPosition().getZ());
