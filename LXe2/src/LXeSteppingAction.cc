...
void LXeSteppingAction::UserSteppingAction(const G4Step* theStep)
{
  ...
  if (theTrack->GetCreatorModelName()=="model_RDM_BetaMinus" && theTrack->GetDefinition()->GetParticleName()=="e-"){
    fEventAction->SetPrimaryeE(theTrack->GetVertexKineticEnergy()/keV);
//      fEventAction->SetPrimaryeE(theTrack->GetKineticEnergy() / keV);
//    theTrack->GetKineticEnergy() / keV
//    G4cout<<theTrack->GetKineticEnergy() / keV<<G4endl;
  }



//  if (theStep->GetTotalEnergyDeposit()>0 && theTrack->GetDefinition()->GetParticleType()!=G4String("opticalphoton") && theTrack->GetDefinition()->GetParticleName()!=G4String("e-"))
