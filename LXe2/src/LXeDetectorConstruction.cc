...
#include "NistElementBuilderMod.hh"
...
LXeDetectorConstruction::LXeDetectorConstruction()
    : fParser(), fLXe_mt(nullptr),fLXe_mt2(nullptr)
    , fMPTPStyrene(nullptr)
{
    ...
    fLXe = fLXe2 = fAl = fAir = fVacuum = fGlass = nullptr;
...
//    fLXe = nist->FindOrBuildMaterial("G4_SODIUM_IODIDE");
//    fLXe->SetName("NaITl");

//    fLXe2 = new G4Material("NaITlLi", density = 3.667*g/cm3, 3);
    fLXe2 = new G4Material("NaITlLi", density = 0.00001*g/cm3, 3);
    fLXe2->AddElement(nist->FindOrBuildElement("Na"), 99);
    fLXe2->AddElement(nist->FindOrBuildElement("I"), 99);
    //Modify NIST data base for contructing enriched Li
    G4NistElementBuilderMod nistMod(0);
    G4Element* enrichLi = nistMod.FindOrBuildElement("Li");
    fLXe2->AddElement(enrichLi, 2);
    fLXe2->GetIonisation()->SetMeanExcitationEnergy(452.*eV);//From NIST
...
//    //! Set Teflon as a reflector
//    G4MaterialPropertiesTable* teflonMPT = new G4MaterialPropertiesTable();
//    teflonMPT->AddProperty("REFLECTIVITY", al_ephoton, reflectivity);
//    teflonMPT->AddProperty("EFFICIENCY", al_ephoton, efficiency);
...
void LXeDetectorConstruction::SetMainScintYield(G4double y)
{
    ...
    fLXe_mt2->AddConstProperty("SCINTILLATIONYIELD", y / keV);
}
