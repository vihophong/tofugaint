...
//      G4cout<<scintNo<<"\t"<<scintPos.getX()<<"\t"<<scintPos.getY()<<"\t"<<scintPos.getZ()<<G4endl;
      edep = (*scintHC)[i]->GetEdep();
      timeAve = (*scintHC)[i]->GetT();
//      timeAve = (*scintHC)[i]->GetTpre();

//      if (i<MAX_SCINT_MULT){
//          evtdata->sid[i] = scintNo;
//          evtdata->se[i] = edep;
//          evtdata->st[i] = edep;
//          evtdata->sx[i] = (*scintHC)[i]->GetPos().getX();
//          evtdata->sy[i] = (*scintHC)[i]->GetPos().getY();
//          evtdata->sz[i] = (*scintHC)[i]->GetPos().getZ();
//      }
...
//      G4cout<<pmtNo<<"\t"<<pmtPost.getX()<<"\t"<<pmtPost.getY()<<"\t"<<pmtPost.getZ()<<"\t"<<(*pmtHC)[i]->GetPhotonCount()<<G4endl;
