...
TH1F* getH1d(char* infile,char* hname,Int_t nbins,Double_t emax,Int_t nn)
{
    ...
    TH1F* h1 = new TH1F("h1","h1",200,0,100);
    TH1F* h2 = new TH1F("h2","h2",200,0,12);
...
void det_response()
{
    ...
    TFile* outfile = new TFile("eff_histogram.root","recreate");
    ...
    for (Int_t i=0;i<nbins;i++){//true (y)
        ...
        hh[i]->SetLineColor(i);
        hh[i]->SetName(Form("heff%d",200*i+100));
        if (i==1)
            hh[i]->Draw();
        else
            hh[i]->Draw("same");
//        hmatrix->SetBinContent(i+1,)
    ...
    hh[nbins] = getH1d(Form("vary_ene_more/LXe_%d.root",1000),Form("%d",1000),nbins,emax,nbins);
    hh[nbins]->SetName(Form("heff%d",1000));
